# Script PowerShell pour copier la clé SSH sur une instance EC2
# Usage: .\copy-ssh-key.ps1 -HostIP "51.21.196.104" -AWSKey "votre-cle-aws.pem"
...
if (-not (Test-Path "github-actions-key.pub")) {
    ...
    Write-Host "Generez d'abord la cle avec: ssh-keygen -t rsa -b 4096 -C 'github-actions' -f github-actions-key" -ForegroundColor Yellow
    exit 1
}
